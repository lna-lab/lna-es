// LNA-ES v3.0 Emotion Ontology - Complete Emotion System
// Based on Plutchik's emotion wheel and refined for Japanese literary context
// 3-level hierarchy with comprehensive emotional coverage
...
// ========================================
// JAPANESE-SPECIFIC EMOTIONS (Cultural Addition)
// ========================================

// もののあはれ (Mono no aware) - Aesthetic emotion
MERGE (:Emotion {name_en: 'Mono no aware', name_ja: 'もののあはれ', level: 2, cultural: 'japanese'})

// 侘寂 (Wabi-sabi) - Beauty in imperfection
MERGE (:Emotion {name_en: 'Wabi-sabi', name_ja: '侘寂', level: 2, cultural: 'japanese'})

// 幽玄 (Yugen) - Subtle grace
MERGE (:Emotion {name_en: 'Yugen', name_ja: '幽玄', level: 2, cultural: 'japanese'})
...
CREATE INDEX emotion_cultural IF NOT EXISTS FOR (e:Emotion) ON (e.cultural);
...
// HAPPY hierarchy
MATCH (core:Emotion {name_en: 'HAPPY'})
MATCH (peaceful:Emotion {name_en: 'Peaceful'})
MATCH (powerful:Emotion {name_en: 'Powerful'})
MATCH (accepted:Emotion {name_en: 'Accepted'})
MATCH (proud:Emotion {name_en: 'Proud'})
MATCH (interested:Emotion {name_en: 'Interested'})
MATCH (joyful:Emotion {name_en: 'Joyful'})
MERGE (core)-[:CONTAINS]->(peaceful)
MERGE (core)-[:CONTAINS]->(powerful)
MERGE (core)-[:CONTAINS]->(accepted)
MERGE (core)-[:CONTAINS]->(proud)
MERGE (core)-[:CONTAINS]->(interested)
MERGE (core)-[:CONTAINS]->(joyful);

// Add level 2 relationships for HAPPY
MATCH (peaceful:Emotion {name_en: 'Peaceful'})
MATCH (loving:Emotion {name_en: 'Loving'})
MATCH (hopeful:Emotion {name_en: 'Hopeful'})
MATCH (sensitive:Emotion {name_en: 'Sensitive'})
MATCH (playful:Emotion {name_en: 'Playful'})
MERGE (peaceful)-[:CONTAINS]->(loving)
MERGE (peaceful)-[:CONTAINS]->(hopeful)
MERGE (peaceful)-[:CONTAINS]->(sensitive)
MERGE (peaceful)-[:CONTAINS]->(playful);
